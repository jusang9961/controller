...
<body>
	<%
		...
		SmartbillOldDAO SmartbillOldDAO = new SmartbillOldDAO();
		String userID = null;
		
		//conversation_ID 랜덤값 생성
		StringBuffer ConId = SmartbillOldDAO.Conid();
		...
		int result = SmartbillOldDAO.arissue(Conversation_id, smartbill.getTxtSupComRegNo(), smartbill.getTxtSupComRepName(), smartbill.getTxtSupComName(), smartbill.getTxtSupBizType(), smartbill.getTxtSupClassify(), smartbill.getTxtSupComAddr(), smartbill.getTxtSupDeptName(), smartbill.getTxtSupDeptPerson(), smartbill.getTxtSupDeptTel(), smartbill.getTxtSupDeptEmail(), smartbill.getTxtByrComRegNo(), smartbill.getTxtByrComRepName(), smartbill.getTxtByrComName(), smartbill.getTxtByrBizType(), smartbill.getTxtByrClassify(), smartbill.getTxtByrComAddr(), smartbill.getTxtByrDeptName(), smartbill.getTxtByrDeptPerson(), smartbill.getTxtByrDeptTel(), smartbill.getTxtByrDeptEmail(), smartbill.getTxtBigo(), smartbill.getTxtSupAmount(), smartbill.getTxtTaxAmount(), smartbill.getTxtTotalAmount(), Batchid);
		int result1 = SmartbillOldDAO.arissueItem(Conversation_id, smartbill.getTxtSupAmount(), smartbill.getTxtTaxAmount());
		int result2 = SmartbillOldDAO.arissueStatus(Conversation_id);
		...
		script.println("</script>");
				...
				PrintWriter script = response.getWriter();
				script.println("<script>");
				script.println("location.href = 'http://localhost:10001/callSB_V3/XXSB_DTI_ARISSUE.asp?BATCH_ID="+Batchid+"&ID=29test&PASS=111'");
				script.println("</script>");
			}
