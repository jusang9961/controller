<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="smartbillOld.SmartbillOldReceiveDAO"%>
...
<jsp:useBean id="smartbill" class="smartbillOld.SmartbillOldReceive"
	scope="page" />
...
<body>
	<%
		SmartbillOldReceiveDAO SmartbillOldReceiveDAO = new SmartbillOldReceiveDAO();

		String userID = null;

		//conversation_ID 랜덤값 생성
		StringBuffer ConId = SmartbillOldReceiveDAO.Conid();
		String Conversation_id = smartbill.getTxtSupComRegNo() + smartbill.getTxtByrComRegNo()
				+ smartbill.getTxtDate() + ConId + "003";
		...
		if (userID == null) {
			...
			if (smartbill.getTxtSupComName() == null || smartbill.getTxtSupComRepName() == null
				...
				int result = SmartbillOldReceiveDAO.arissue(Conversation_id,
						smartbill.getTxtSupComRegNo(), smartbill.getTxtSupComRepName(),
						smartbill.getTxtSupComName(), smartbill.getTxtSupBizType(), smartbill.getTxtSupClassify(),
						smartbill.getTxtSupComAddr(), smartbill.getTxtSupDeptName(),
						smartbill.getTxtSupDeptPerson(), smartbill.getTxtSupDeptTel(),
						smartbill.getTxtSupDeptEmail(), smartbill.getTxtByrComRegNo(),
						smartbill.getTxtByrComRepName(), smartbill.getTxtByrComName(), smartbill.getTxtByrBizType(),
						smartbill.getTxtByrClassify(), smartbill.getTxtByrComAddr(), smartbill.getTxtByrDeptName(),
						smartbill.getTxtByrDeptPerson(), smartbill.getTxtByrDeptTel(),
						smartbill.getTxtByrDeptEmail(), smartbill.getTxtBigo(), smartbill.getTxtSupAmount(),
						smartbill.getTxtTaxAmount(), smartbill.getTxtTotalAmount(), Batchid);
				int result1 = SmartbillOldReceiveDAO.arissueItem(Conversation_id,
						smartbill.getTxtSupAmount(), smartbill.getTxtTaxAmount());
				int result2 = SmartbillOldReceiveDAO.arissueStatus(Conversation_id);
				if (result == -1 || result1 == -1 || result2 == -1) {
					...
					PrintWriter script = response.getWriter();
					script.println("<script>");
 					script.println("location.href = 'http://localhost:10001/callSB_V3/XXSB_DTI_ISSUE.asp?BATCH_ID="
 							+ Batchid + "&ID=29test&PASS=1111'");
					script.println("</script>");
				}
